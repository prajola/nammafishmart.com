    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-3">
          {freeDeliveryLeft > 0 && (
            <div className="rounded-2xl bg-white/5 px-4 py-3 text-sm font-semibold text-brand-700">
          ...
          {cartProducts.map(({ product, qty }) => (
            <div
              key={product.id}
              className="flex gap-4 rounded-2xl border border-white/10 bg-navy-800 p-3 shadow-[var(--shadow-soft)]"
            >
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                ...
                <div className="mt-auto flex items-center justify-between pt-2">
                  <div className="flex items-center gap-2 rounded-full bg-white/5 p-0.5 ring-1 ring-brand-200">
                    <button
                      onClick={() => setQty(product.id, qty - 1)}
                      className="grid h-8 w-8 place-items-center rounded-full text-brand-700 hover:bg-navy-800"
                    >
                      −
                    </button>
        ...
        <aside className="h-fit space-y-4 lg:sticky lg:top-24">
          {/* Coupon */}
          <div className="rounded-2xl border border-white/10 bg-navy-800 p-4 shadow-[var(--shadow-soft)]">
              ...
              <>
                <div className="flex gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-xl border border-white/15 px-3 py-2.5 text-sm uppercase outline-none focus:border-brand-400"
                  />
                  <button
                    onClick={tryCoupon}
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {Object.entries(COUPONS).map(([c, v]) => (
                    <button
                      ...
                      className="rounded-lg border border-dashed border-brand-300 bg-white/5 px-2 py-1 text-[11px] font-bold text-brand-700 hover:bg-brand-100"
                      title={v.label}
                    >
                      {c}
                    </button>
